...
- **Touch controls:** Add second-tap protection to Follow and Like. Share confirmation also covers accessibility actions and keyboard input. If Hushfeed cannot prepare the first confirmation, it stops that tap instead of sending. Remap or disable long press and double tap.
...
| `Downloads` | Adds watermark-free downloads, comment sticker saving, configurable folders, and filename templates. |
...
| `Hide inbox stories` | Stops the stories tray at the top of the Inbox from being built. Shares its switch with Hide inbox items. |
