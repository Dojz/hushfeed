...
* **TikTok:** New Swipe-left controls patch. A left swipe on a feed video opens the creator's profile in TikTok, and the new Swipe left row under Feed screen > Gestures can make it do nothing or open the video's comments instead. From the profile, swiping back to the feed works as before, and a photo post's own swipe between pictures is left alone. Changes take effect without a restart.
